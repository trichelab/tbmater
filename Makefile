...
test:
	...
	R --slave -e 'Sys.setenv(NOT_CRAN="true"); library(testthat); setwd(file.path(.libPaths()[1], "tbmater", "tests")); system.time(test_check("tbmater", filter="${file}", reporter=ifelse(nchar("${r}"), "${r}", "summary")))'
...
check: build
	-R CMD check --as-cran tbmater_$(VERSION).tar.gz
	rm -rf tbmater.Rcheck/
